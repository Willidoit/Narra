...
/// OpenAI Whisper / GPT-4o transcription endpoint.
/// Same multipart shape as Grok — different host and a few model IDs.
public final class OpenAITranscriptionService: TranscriptionService, @unchecked Sendable {
    ...
    public init(
        ...
        self.apiKey = apiKey ?? KeychainService.load(for: .openAI) ?? ""
        self.session = session
    }

    public func transcribe(audio: AudioChunk) async throws -> TranscriptSegment {
        guard !apiKey.isEmpty else {
            throw TranscriptionError.serviceError("Missing OpenAI API key")
        }
        ...
        if let hint = configuration.languageHint {
            fields.append(.text(name: "language", value: hint))
        }
        ...
        guard (200..<300).contains(http.statusCode) else {
            ...
            throw TranscriptionError.serviceError("OpenAI HTTP \(http.statusCode): \(snippet)")
        }
